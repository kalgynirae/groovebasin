...
function Player(db, config) {
  ...
  this.grooveEncoder.bitRate = config.bitRate;
  ...
  this.autoPause = config.autoPause;
}
...
Player.prototype.refreshFilesIndex = function(args, cb) {
  ...
  walker.on('file', function(fullPath, stat, linkPath) {
    ...
    onAddOrChange(self, relPath, fileMtime, forceRescan);
  });
...
Player.prototype.lastStreamerDisconnected = function() {
  ...
  if (!this.desiredPlayerHardwareState && this.isPlaying && this.autoPause) {
    this.emit("autoPause");
    this.pause();
  }
};
...
Player.prototype.shufflePlaylist = function() {
  ...
  var sortKeys = keese(null, null, this.tracksInOrder.length);
  var updateCmds = [];
...
function onAddOrChange(self, relPath, fileMtime, forceRescan, cb) {
  cb = cb || logIfError;
  ...
  groove.open(srcFullPath, function(err, file) {
    ...
    function tryMv(destRelPath, cb) {
      ...
      mv(srcFullPath, destFullPath, {mkdirp: true, clobber: false}, function(err) {
        ...
        onAddOrChange(self, destRelPath, (new Date()).getTime(), true, function(err, dbFile) {
          if (err) return cb(err);
          newDbFile = dbFile;
          cb();
        });
